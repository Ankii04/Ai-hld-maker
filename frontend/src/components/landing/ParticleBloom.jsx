...
export default function ParticleBloom({ className = '', count = 5200, offsetX = 0, offsetY = 0, scrollReactive = false }) {
  ...
  useEffect(() => {
    ...
    const render = (time) => {
      ...
      // On narrow/short viewports the bloom's footprint (radius ≈ scale)
      // covers most of the screen, sprawling straight into the CTA button
      // and tag row below the fold. Shrink it, dim it, and pull its center
      // up toward the headline so it stays a headline accent instead of
      // fighting the buttons for contrast.
